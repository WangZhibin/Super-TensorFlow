...
void BuildBuildGraphOptions(const RunStepRequestWrapper& req,
  ...
  opts->disable_mixed_precision =
      req.options().experimental().disable_mixed_precision();
  opts->disable_xla =
      req.options().experimental().disable_xla();
}
...
uint64 HashBuildGraphOptions(const BuildGraphOptions& opts) {
  ...
  string precision = "mixed";
  if (opts.disable_mixed_precision) {
    precision = "full";
  }
  h = Hash64(precision.c_str(), precision.size(), h);
  if (opts.disable_xla) {
    string disable_xla = "disable_xla";
    h = Hash64(disable_xla.c_str(), disable_xla.size(), h);
  }
