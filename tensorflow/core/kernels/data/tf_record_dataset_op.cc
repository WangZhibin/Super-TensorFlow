...
// tdw
class TDWRecordDatasetOp::Dataset : public DatasetBase {
 public:
    explicit Dataset(OpKernelContext* ctx, std::vector<string> filenames)
            : DatasetBase(DatasetContext(ctx)),
              filenames_(std::move(filenames)) {
    ...
    std::unique_ptr<IteratorBase> MakeIteratorInternal(
        ...
        return std::unique_ptr<IteratorBase>(
                new Iterator({this, strings::StrCat(prefix, "::TDWRecord")}));
    }
    ...
    string DebugString() const override { return "TDWRecordDatasetOp::Dataset"; }
 ...
 private:
    ...
    public:
        ...
        Status GetNextInternal(IteratorContext* ctx,
            ...
            do {
                ...
                tdw::TDWRecordReader* reader = tdw_client->GetRecordReader(next_filename);
                if (reader == nullptr) {
    ...
    private:
        ...
        std::unique_ptr<tdw::TDWRecordReader> reader_ GUARDED_BY(mu_);
    };

    const std::vector<string> filenames_;
};

TDWRecordDatasetOp::TDWRecordDatasetOp(OpKernelConstruction* ctx)
    : DatasetOpKernel(ctx) {}

void TDWRecordDatasetOp::MakeDataset(OpKernelContext* ctx,
                                    DatasetBase** output) {
    const Tensor* filenames_tensor;
...
REGISTER_KERNEL_BUILDER(Name("TFRecordDataset").Device(DEVICE_CPU),
                        TFRecordDatasetOp);
REGISTER_KERNEL_BUILDER(Name("TDWRecordDataset").Device(DEVICE_CPU),
                        TDWRecordDatasetOp);
}  // namespace
}  // namespace data
}  // namespace tensorflow
