...
#ifdef TENSORFLOW_USE_SAGE2
#include "tensorflow/core/lib/sage2/include/sage2/tf_optimizer.h"
#endif
...
#ifdef TENSORFLOW_USE_SAGE2
template <>
struct ApplyFtrl<CPUDevice, float> {
  void operator()(const CPUDevice& d, typename TTypes<float>::Flat var,
                  typename TTypes<float>::Flat accum,
                  typename TTypes<float>::Flat linear,
                  typename TTypes<float>::ConstFlat grad,
                  typename TTypes<float>::ConstScalar lr,
                  typename TTypes<float>::ConstScalar l1,
                  typename TTypes<float>::ConstScalar l2,
                  typename TTypes<float>::ConstScalar lr_power) {
    float _lr_power = lr_power();
    if (_lr_power == -0.5f) {
      // special case for which lr_power=-0.5.
      sage2_tf_ftrl_config_v1_s config;
      config.alpha = lr();
      config.l1 = l1();
      config.l2 = 2.0f * l2();
      config.inv_alpha = 1.0f / lr();
      sage2_tf_ftrl_update_v1_ps(&config, var.size(), grad.data(), var.data(),
                                 accum.data(), linear.data());
    } else {
      auto grad_square = grad.square();
      auto new_accum = accum + grad_square;
      linear.device(d) +=
          grad -
          (new_accum.pow(-lr_power()) - accum.pow(-lr_power())) / lr() * var;
      auto x = (linear.constant(l1()) * linear.sign() - linear);
      auto y = new_accum.pow(-lr_power()) / new_accum.constant(lr()) +
               linear.constant(static_cast<float>(2) * l2());
      auto pre_shrink = x / y;
      var.device(d) =
          (linear.abs() > linear.constant(l1()))
              .select(pre_shrink, var.constant(static_cast<float>(0)));
      accum.device(d) += grad_square;
    }
  }
};
#endif
 ...
 public:
...
#if GOOGLE_CUDA
  ~ApplyAdagradHashOp() { cudaStreamDestroy(stream); }
#endif
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if (IsGPUDevice<Device>()) {
      ...
      OP_REQUIRES_OK(
          ctx, ctx->allocate_output(0, var.shape(), &new_var, alloc_attrs));
      OP_REQUIRES_OK(
          ctx, ctx->allocate_output(1, accum.shape(), &new_accum, alloc_attrs));
      cudaMemcpy((void*)new_var->flat<T>().data(), (void*)var.flat<T>().data(),
                 num_elements * sizeof(T), cudaMemcpyDefault);
      cudaMemcpy((void*)new_accum->flat<T>().data(),
                 (void*)accum.flat<T>().data(), num_elements * sizeof(T),
                 cudaMemcpyDefault);
    } else
#endif
    {
      ...
      std::copy_n(var.flat<T>().data(), num_elements,
                  new_var->flat<T>().data());
      std::copy_n(accum.flat<T>().data(), num_elements,
                  new_accum->flat<T>().data());
    }
    const Device& device = ctx->template eigen_device<Device>();
    functor::ApplyAdagrad<Device, T>()(device, new_var->flat<T>(),
                                       new_accum->flat<T>(), lr.scalar<T>(),
...
class ApplyFtrlHashOp : public OpKernel {
 public:
  explicit ApplyFtrlHashOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
    OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
  }
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    std::copy_n(accum.flat<T>().data(), num_elements,
                new_accum->flat<T>().data());
    std::copy_n(linear.flat<T>().data(), num_elements,
                new_linear->flat<T>().data());
    ...
    if (has_l2_shrinkage) {
      ...
      OP_REQUIRES(
          ...
          errors::InvalidArgument("l2 shrinkage regularization strength "
                                  ...
                                  l2_shrinkage.shape().DebugString()));
      functor::ApplyFtrlV2<Device, T>()(
          device, new_var->flat<T>(), new_accum->flat<T>(),
          new_linear->flat<T>(), grad.flat<T>(), lr.scalar<T>(), l1.scalar<T>(),
          l2.scalar<T>(), l2_shrinkage.scalar<T>(), lr_power.scalar<T>());
    } else {
      functor::ApplyFtrl<Device, T>()(
          device, new_var->flat<T>(), new_accum->flat<T>(),
          new_linear->flat<T>(), grad.flat<T>(), lr.scalar<T>(), l1.scalar<T>(),
          l2.scalar<T>(), lr_power.scalar<T>());
    }
...
#define REGISTER_KERNELS(D, T)                                         \
  ...
  REGISTER_KERNEL_BUILDER(                                             \
      Name("ApplyFtrlHash").Device(DEVICE_##D).TypeConstraint<T>("T"), \
...
#define REGISTER_KERNELS(D, T)                                           \
  ...
  REGISTER_KERNEL_BUILDER(                                               \
      Name("ApplyFtrlHashV2").Device(DEVICE_##D).TypeConstraint<T>("T"), \
      ApplyFtrlHashOp<D##Device, T, /*has_l2_shrinkage=*/true>);         \
  REGISTER_KERNEL_BUILDER(                                               \
...
/***************** Ftrl Gpu **********************/
template <typename Device, typename T, bool has_l2_shrinkage>
class ApplyFtrlGpuHashOp : public OpKernel {
 public:
  explicit ApplyFtrlGpuHashOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
    OP_REQUIRES_OK(ctx, ctx->GetAttr("use_locking", &use_exclusive_lock_));
  }
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES_OK(ctx,
                   ctx->allocate_output(0, var.shape(), &new_var, alloc_attrs));
    OP_REQUIRES_OK(
        ctx, ctx->allocate_output(1, accum.shape(), &new_accum, alloc_attrs));
    OP_REQUIRES_OK(
        ctx, ctx->allocate_output(2, linear.shape(), &new_linear, alloc_attrs));
    cudaMemcpy((void*)new_var->flat<T>().data(), (void*)var.flat<T>().data(),
               num_elements * sizeof(T), cudaMemcpyDeviceToDevice);
    cudaMemcpy((void*)new_accum->flat<T>().data(),
               (void*)accum.flat<T>().data(), num_elements * sizeof(T),
               cudaMemcpyDeviceToDevice);
    cudaMemcpy((void*)new_linear->flat<T>().data(),
               (void*)linear.flat<T>().data(), num_elements * sizeof(T),
               cudaMemcpyDeviceToDevice);
#else
    OP_REQUIRES_OK(ctx, ctx->allocate_output(0, var.shape(), &new_var));
    OP_REQUIRES_OK(ctx, ctx->allocate_output(1, accum.shape(), &new_accum));
...
#define REGISTER_KERNELS(D, T)                                         \
  REGISTER_KERNEL_BUILDER(                                             \
      Name("ApplyFtrlHash").Device(DEVICE_##D).TypeConstraint<T>("T"), \
      ApplyFtrlGpuHashOp<D##Device, T, /*has_l2_shrinkage=*/false>);
...
#define REGISTER_KERNELS(D, T)                                           \
  REGISTER_KERNEL_BUILDER(                                               \
      Name("ApplyFtrlHashV2").Device(DEVICE_##D).TypeConstraint<T>("T"), \
      ApplyFtrlGpuHashOp<D##Device, T, /*has_l2_shrinkage=*/true>);      \
  REGISTER_KERNEL_BUILDER(                                               \
      Name("ResourceApplyFtrlV2")                                        \
          .HostMemory("var")                                             \
          .HostMemory("accum")                                           \
...
class ApplyMomentumHashOp : public OpKernel {
 public:
...
#if GOOGLE_CUDA
  ~ApplyMomentumHashOp() { cudaStreamDestroy(stream); }
#endif
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    const Device& device = ctx->template eigen_device<Device>();
    functor::ApplyMomentum<Device, T>()(
        device, new_var->flat<T>(), new_accum->flat<T>(), lr.scalar<T>(),
        grad.flat<T>(), momentum.scalar<T>(), use_nesterov_);
  }
...
#if GOOGLE_CUDA
  ~ApplyAdamHashOp() { cudaStreamDestroy(stream); }
#endif
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if (IsGPUDevice<Device>()) {
      ...
      OP_REQUIRES_OK(
          ctx, ctx->allocate_output(0, var.shape(), &new_var, alloc_attrs));
      OP_REQUIRES_OK(ctx,
                     ctx->allocate_output(1, m.shape(), &new_m, alloc_attrs));
      OP_REQUIRES_OK(ctx,
                     ctx->allocate_output(2, v.shape(), &new_v, alloc_attrs));
      cudaMemcpy((void*)new_var->flat<T>().data(), (void*)var.flat<T>().data(),
                 num_elements * sizeof(T), cudaMemcpyDefault);
      cudaMemcpy((void*)new_m->flat<T>().data(), (void*)m.flat<T>().data(),
                 num_elements * sizeof(T), cudaMemcpyDefault);
      cudaMemcpy((void*)new_v->flat<T>().data(), (void*)v.flat<T>().data(),
                 num_elements * sizeof(T), cudaMemcpyDefault);
      // cudaStreamSynchronize(stream);
    } else
#endif
