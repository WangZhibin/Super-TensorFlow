...
class LibHDFS {
 public:
  ...
  std::function<int(struct hdfsBuilder *, const char*, const char *)>hdfsBuilderConfSetStr;
...
#define BIND_HDFS_FUNC(function) \
  TF_RETURN_IF_ERROR(BindFunc(*handle, #function, &function));
      ...
      BIND_HDFS_FUNC(hdfsBuilderConfSetStr);
...
Status HadoopFileSystem::Connect(StringPiece fname, hdfsFS* fs) {
  ...
  char* hadoop_job_ugi = getenv("hadoop_job_ugi");
    if(hadoop_job_ugi != nullptr){
      hdfs_->hdfsBuilderConfSetStr(builder,"hadoop.job.ugi",hadoop_job_ugi);
  }
 ...
 public:
  HDFSRandomAccessFile(const string& filename, const string& hdfs_filename,
      ...
      : filename_(filename),
        ...
        fs_(fs),
        file_(file) {
    const char* disable_read_eof_retried = getenv("HDFS_DISABLE_READ_EOF_RETRIED");
    if (disable_read_eof_retried && strcmp(disable_read_eof_retried, "1") == 0) {
  ...
  Status Read(uint64 offset, size_t n, StringPiece* result,
    ...
    if (disable_eof_retried_) {
      // avoid calling hdfsOpenFile in Read, Fixes #42597
      eof_retried = true;
    }
    while (n > 0 && s.ok()) {
