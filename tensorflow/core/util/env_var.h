...
// Returns an size_t into "value" from the environmental variable
// "env_var_name". If it is unset, the default value is used. If the string
// cannot be parsed into size_t, an error status is returned.
Status ReadSizetFromEnvVar(StringPiece env_var_name, size_t default_val,
                           size_t* value);
