...
/************************************************************************/
template <typename T>
class BatchFMInteraction2Op : public OpKernel {
 private:
  using ll_math_t = deepx_core::LLMath<T>;

 public:
  explicit BatchFMInteraction2Op(OpKernelConstruction* c) : OpKernel(c) {}
...
REGISTER_KERNEL_BUILDER(
    Name("BatchFMInteraction2").Device(DEVICE_CPU).TypeConstraint<float>("T"),
    BatchFMInteraction2Op<float>);

REGISTER_KERNEL_BUILDER(
    Name("BatchFMInteraction2").Device(DEVICE_CPU).TypeConstraint<double>("T"),
    BatchFMInteraction2Op<double>);

/************************************************************************/
/* BatchFMInteraction2Grad */
/************************************************************************/
template <typename T>
class BatchFMInteraction2GradOp : public OpKernel {
 private:
  using ll_math_t = deepx_core::LLMath<T>;

 public:
  explicit BatchFMInteraction2GradOp(OpKernelConstruction* c) : OpKernel(c) {}
...
REGISTER_KERNEL_BUILDER(Name("BatchFMInteraction2Grad")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<float>("T"),
                        BatchFMInteraction2GradOp<float>);

REGISTER_KERNEL_BUILDER(Name("BatchFMInteraction2Grad")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<double>("T"),
                        BatchFMInteraction2GradOp<double>);
