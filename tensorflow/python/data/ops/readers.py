...
class _TDWRecordDataset(dataset_ops.Dataset):
  def __init__(self, filenames):
      """Creates a `TDWRecordDataset`.
      ...
      super(_TDWRecordDataset, self).__init__()

  def _as_variant_tensor(self):
      return gen_dataset_ops.tdw_record_dataset(
          self._filenames)
...
@tf_export("data.TDWRecordDataset", v1=[])
class TDWRecordDatasetV2(dataset_ops.DatasetV2):
    """A `Dataset` comprising records from one or more TDWRecord files."""

    def __init__(self, filenames, num_parallel_reads=None):
        """Creates a `TDWRecordDataset` to read one or more TDWRecord files.
        ...
        def creator_fn(filename):
            return _TDWRecordDataset(filename)

        self._impl = _create_dataset_reader(creator_fn, filenames,
                                            num_parallel_reads)
        variant_tensor = self._impl._variant_tensor  # pylint: disable=protected-access
        super(TDWRecordDatasetV2, self).__init__(variant_tensor)

    def _clone(self,
               filenames=None,
               num_parallel_reads=None):
        return TDWRecordDatasetV2(filenames or self._filenames,
                                 num_parallel_reads or self._num_parallel_reads)
...
@tf_export(v1=["data.TDWRecordDataset"])
class TDWRecordDatasetV1(dataset_ops.DatasetV1Adapter):
    """A `Dataset` comprising records from one or more TDWRecord files."""

    def __init__(self, filenames, num_parallel_reads=None):
        wrapped = TDWRecordDatasetV2(
            filenames, num_parallel_reads)
        super(TDWRecordDatasetV1, self).__init__(wrapped)
    __init__.__doc__ = TDWRecordDatasetV2.__init__.__doc__

    def _clone(self,
               filenames=None,
               num_parallel_reads=None):
        # pylint: disable=protected-access
        return TDWRecordDatasetV1(
            filenames or self._dataset._filenames,
            num_parallel_reads or self._dataset._num_parallel_reads)
...
TDWRecordDataset = TDWRecordDatasetV1
