...
def create_meta_graph_def(meta_info_def=None,
  ...
  for ctype in clist:
    ...
    elif ctype == ops.GraphKeys.DYNAMIC_EMBEDDING_VARIABLES:
      # TODO(chuancheng): hack to support import and export meta graph for DYNAMIC_EMBEDDING_VARIABLES items.
      if graph:
        devar_names = [v.name for v in graph.get_collection(ctype)]
      else:
        devar_names = [v.name for v
                       in ops.get_default_graph().get_collection(ctype)]
      add_collection_def(meta_graph_def, ctype,
                         graph=graph,
                         export_scope=export_scope,
                         exclude_nodes=exclude_nodes,
                         override_contents=devar_names)
    else:
      add_collection_def(meta_graph_def, ctype,
                         graph=graph,
                         export_scope=export_scope,
  ...
  with graph.as_default():
    ...
    for key, col_def in sorted_collections:
      ...
      if from_proto and kind == "bytes_list":
        ...
        if key == ops.GraphKeys.DYNAMIC_EMBEDDING_VARIABLES:
          # TODO(chuancheng): hack to support import and export meta graph for DYNAMIC_EMBEDDING_VARIABLES items.
          field = getattr(col_def, kind)
          for value in field.value:
            graph.add_to_collection(
              key,
              ops.prepend_name_scope(compat.as_str(value),
