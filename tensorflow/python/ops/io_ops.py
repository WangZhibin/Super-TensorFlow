...
@tf_export(v1=["TDWRecordReader"])
class TDWRecordReader(ReaderBase):
    """A Reader that outputs the records from a tdw table file.

    See ReaderBase for supported methods.
    """

    def __init__(self, name=None, field_indices=None):
        """Create a TFRecordReader.

        Args:
          name: A name for the operation (optional).
          field_indices: The required field indices based on tdw table (optional).
        """
        rr = gen_io_ops.tdw_record_reader(
            name=name, field_indices=field_indices)
        super(TDWRecordReader, self).__init__(rr)

ops.NotDifferentiable("TDWRecordReader")
