...
__all__ = [
    ...
    "init_mht_saveable_from_checkpoint"
]
...
@tf_export(v1=["train.init_mht_saveable_from_checkpoint"])
def init_mht_saveable_from_checkpoint(ckpt_dir_or_file, assignment_map):
  init_from_checkpoint_fn = lambda _: _init_mht_saveable_from_checkpoint(
    ckpt_dir_or_file, assignment_map)
  if distribution_strategy_context.get_cross_replica_context():
    init_from_checkpoint_fn(None)
  else:
...
def _init_mht_saveable_from_checkpoint(ckpt_dir_or_file, assignment_map):
  """See `init_mht_saveable_from_checkpoint` for documentation."""
  ckpt_file = _get_checkpoint_filename(ckpt_dir_or_file)
  reader = load_checkpoint(ckpt_dir_or_file)
  variable_map = reader.get_variable_to_shape_map()
  for saveable_name_in_ckpt, current_saveable_or_name in sorted(
      six.iteritems(assignment_map)):
    if _is_mht_saveable(current_saveable_or_name):
      saveable = current_saveable_or_name
    else:
      _saveables = ops.get_collection(ops.GraphKeys.SAVEABLE_OBJECTS,
                                      scope=current_saveable_or_name)
      if _saveables:
        saveable = _saveables[0]
      else:
        raise ValueError("MutableHashTable._Saveable with name {} is not found "
                         "in graph".format(current_saveable_or_name))
    if len(saveable.specs) != 2:
      raise ValueError(
        "Specs size of MutableHashTable._Saveable must be 2, which "
        "is %d" % len(saveable.specs))

    keys_name_in_ckpt = saveable_name_in_ckpt + "-keys"
    values_name_in_ckpt = saveable_name_in_ckpt + "-values"
    for tensor, name_in_ckpt in zip([spec.tensor for spec in saveable.specs],
                                    [keys_name_in_ckpt, values_name_in_ckpt]):
      if name_in_ckpt not in variable_map:
        raise ValueError("Tensor %s is not found in %s checkpoint %s" %
                         (name_in_ckpt, ckpt_dir_or_file, variable_map))
      if not tensor.get_shape().is_compatible_with(variable_map[name_in_ckpt]):
        raise ValueError(
          "Shape of variable %s (%s) doesn't match with shape of "
          "tensor %s (%s) from checkpoint reader." %
          (tensor.name, str(tensor.get_shape()), tensor.name,
           str(variable_map[name_in_ckpt])))
    _set_mht_saveable_checkpoint_initializer(saveable, ckpt_file,
                                             saveable_name_in_ckpt)
    logging.debug("Initialize variable %s from checkpoint %s with %s",
                  saveable.name, ckpt_dir_or_file, saveable_name_in_ckpt)
...
def _is_trainable_wrapper(x):
  return isinstance(x, resource_variable_ops.TrainableWrapper)


def _is_dynamic_embedding_variable(x):
  return isinstance(x, deo.Variable)


def _is_mht_saveable(x):
  return isinstance(x, lookup_ops.MutableHashTable._Saveable)  # pylint:disable=protected-access
