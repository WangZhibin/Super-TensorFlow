...
class FtrlOptimizer(optimizer.Optimizer):
  ...
  def _apply_dense_hash(self, grad, var, accum, linear):
    """The stateless version of `_apply_dense`.

    The inputs will be read-only Tensors, and
    the upgraded values of var, accum and linear will be returned.

    Args:
      grad: the gradients tensor.
      var: the variable tensor to be updated.
      accum: the tensor of `accum` slot.
      linear: the tensor of `linear` slot.

    Returns:
      A tuple of three tensors like `(new_var, new_accum, new_linear)` where
        `new_var` is the new values of `var`.
        `new_accum` is the new values of `accum`.
        `new_linear` is the new values of `linear`.
    """
    if self._l2_shrinkage_regularization_strength <= 0.0:
      return training_ops.apply_ftrl_hash(
          var,
          accum,
          linear,
          grad,
      ...
      return training_ops.apply_ftrl_hash_v2(
          var,
          accum,
          linear,
          grad,
