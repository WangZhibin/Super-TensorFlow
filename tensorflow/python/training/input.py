...
@tf_export(v1=["train.read_tdw_record"])
def read_tdw_record(filename_queue, record_defaults, field_indices=None):
    """Read from tdw table files

    Args:
      filename_queue: the filename queue produced by string_input_producer
      record_defaults: default record
      field_indices: The required field indices, [0, 1, 2] or "0,1,2"

    Returns:
      record tensor, e.g. [field1, field2, ...]

    """
    str_field_indices = ""
    if field_indices is not None:
        if isinstance(field_indices, str):
            str_field_indices = field_indices
        elif isinstance(field_indices, list):
            str_field_indices = ",".join(str(x) for x in field_indices)
        else:
            raise ValueError("field_indices must be a str('0,1,2') or list int([0,1,2])")

    reader = io_ops.TDWRecordReader(field_indices=str_field_indices)
    key, value = reader.read(filename_queue)
    record = parsing_ops.decode_csv(value, record_defaults, "\01")
    return record