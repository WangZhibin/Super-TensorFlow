...
def lamb_update_numpy(param,
  ...
  adjusted_grad = _m_t / (np.sqrt(_v_t) + epsilon) + weight_decay * param

  r_1 = linalg.norm(param, ord=2)
  r_2 = linalg.norm(adjusted_grad, ord=2)
  ratio = np.where(r_1 > 0, np.where(r_2 > 0, (r_1 / (r_2 + 1.0e-6)), 1.0), 1.0)

  param_t = param - ratio * alpha * adjusted_grad
  return param_t, m_t, v_t

class LAMBOptimizerTest(test.TestCase):
  ...
  def testSparseDevicePlacement(self):
    for index_dtype in [dtypes.int32, dtypes.int64]:
      with self.cached_session(force_gpu=test.is_gpu_available()):
        ...
        optimizer = lamb.LAMBOptimizer(learning_rate=3.0)
    ...
    for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
      with self.cached_session():
        ...
        grad_aggregated = ops.IndexedSlices(
            ...
            constant_op.constant([2, 1]))
        repeated_update = lamb.LAMBOptimizer().apply_gradients(
            [(grad_repeated_index, repeated_index_update_var)])
        aggregated_update = lamb.LAMBOptimizer().apply_gradients(
  ...
  def doTestBasic(self, use_resource=False, use_callable_params=False):
    ...
    for i, dtype in enumerate([dtypes.half, dtypes.float32, dtypes.float64]):
      with self.session(graph=ops.Graph()):
        ...
        opt = lamb.LAMBOptimizer(learning_rate=learning_rate)
        update = opt.apply_gradients(zip([grads0, grads1], [var0, var1]))
        ...
        for t in range(1, 4):
          ...
          self.assertAllCloseAccordingToType(var1_np, self.evaluate(var1))
          if use_resource:
            self.assertEqual("var0_%d/LAMB:0" % (i,),
  ...
  def testTensorLearningRate(self):
    for dtype in [dtypes.half, dtypes.float32, dtypes.float64]:
      with self.cached_session():
        ...
        grads0 = constant_op.constant(grads0_np)
        grads1 = constant_op.constant(grads1_np)
        opt = lamb.LAMBOptimizer(constant_op.constant(0.001))
        update = opt.apply_gradients(zip([grads0, grads1], [var0, var1]))
        variables.global_variables_initializer().run()
  ...
  def testSlotsUniqueEager(self):
    with context.eager_mode():
      ...
      opt = lamb.LAMBOptimizer(learning_rate=1.)
