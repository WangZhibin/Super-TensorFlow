...
def _get_grouped_variables(vars_to_warm_start):
  ...
  if isinstance(vars_to_warm_start, str) or vars_to_warm_start is None:
    ...
    list_of_vars = [
      v for v in ops.get_collection(ops.GraphKeys.TRAINABLE_VARIABLES,
                                    scope=vars_to_warm_start)
      if not checkpoint_utils._is_trainable_wrapper(v)
    ]
...
# mht is an abbreviation for MutableHashTable.
def _get_grouped_mht_saveables(devars_to_warm_start):
  """Collects and groups MutableHashTable._Saveables into a dictionary.

  The dynamic embedding variables can be provided explicitly through
  vars_to_warm_start, or they are retrieved from collections (see below).

  Args:
    devars_to_warm_start: One of the following:
      - A list of strings, each representing a full variable name to warm-start.
        These will consider dynamic embedding variables in
        DYNAMIC_EMBEDDING_VARIABLES collection.
      - A list of dynamic embedding Variables to warm-start.


  Returns:
    A dictionary mapping MutableHashTable._Saveable names (strings) to lists of
    MutableHashTable._Saveables.
  Raises:
    ValueError: If vars_to_warm_start is not a list of strings, or a list of
      `dynamic embedding Variables`.
  """
  if all(isinstance(v, str) for v in devars_to_warm_start):
    list_of_mht_saveables = []
    for v in devars_to_warm_start:
      devars = ops.get_collection(ops.GraphKeys.DYNAMIC_EMBEDDING_VARIABLES,
                                  scope=v)
      list_of_mht_saveables += [
        table.saveable for v in devars for table in v.tables
      ]
  elif all(checkpoint_utils._is_dynamic_embedding_variable(v)
           for v in devars_to_warm_start): # pylint: disable=protected-access
    list_of_mht_saveables = [
      table.saveable for v in devars_to_warm_start for table in v.tables
    ]
  else:
    raise ValueError(
        "If `vars_to_warm_start` is a list, it must be all "
        "`dynamic_embedding_ops.Variable` or all `str`.  Given types are {}"
            .format([type(v) for v in devars_to_warm_start]))

  grouped_mht_saveables = {}
  for ms in list_of_mht_saveables:
    saveable_name = _infer_mht_saveable_name(ms)
    grouped_mht_saveables.setdefault(saveable_name, []).append(ms)

  return grouped_mht_saveables
...
def warm_start(ckpt_to_initialize_from,
  ...
  Args:
    ...
    vars_to_warm_start: [Optional] One of the following:
      ...
      - A list of strings, each a regex scope provided to
        tf.compat.v1.get_collection with GLOBAL_VARIABLES or
        DYNAMIC_EMBEDDING_VARIABLES (please see tf.compat.v1.get_collection).
        For backwards compatibility reasons,
        this is separate from the single-string argument type.
      - A list of Variables to warm-start.  If you do not have access to the
        `Variable` or `dynamic_embedding_ops.Varaible` objects at the call site,
        please use the above option.
    ...
    var_name_to_prev_var_name: [Optional] Dict of variable names (strings) to
      name of the previously-trained variable in `ckpt_to_initialize_from` or
      MutableHashTable._Saveable names (strings) to name of the
      previously-trained MutableHashTable._Saveable in
      `ckpt_to_initialize_from`. If not explicitly provided, the name of the
      variable is assumed to be same between previous checkpoint and current
      model.  Note that this has no effect on the set of variables that is
      warm-started, and only controls name mapping (use `vars_to_warm_start` for
      controlling what variables to
      warm-start).
  ...
  # devar is an abbreviation for dynamic embedding variable
  vars_to_warm_start, devars_to_warm_start = \
    _separate_vars_and_devars_to_warm_start(vars_to_warm_start)

  if var_name_to_prev_var_name is None:
    var_name_to_prev_var_name = {}

  prev_var_name_used, found_var_names = _warm_start_variables(
    ckpt_to_initialize_from, vars_to_warm_start, var_name_to_vocab_info,
    var_name_to_prev_var_name)

  prev_devar_name_used, found_devar_names = \
    _warm_start_dynamic_embedding_variables(
      ckpt_to_initialize_from, devars_to_warm_start, var_name_to_prev_var_name)

  prev_var_name_not_used = set(var_name_to_prev_var_name.keys()) \
                           - prev_var_name_used - prev_devar_name_used
  if prev_var_name_not_used:
    raise ValueError(
      ...
      "variable names: {1}".format(
        prev_var_name_not_used, found_var_names.union(found_devar_names)))
...
# ms is an abbreviation for MutableHashTable._Saveable
def _warm_start_dynamic_embedding_variables(ckpt_to_initialize_from,
                                            devars_to_warm_start,
                                            ms_name_to_prev_ms_name):
  grouped_mht_saveables = _get_grouped_mht_saveables(devars_to_warm_start)

  if not ms_name_to_prev_ms_name:
    # Detect whether the checkpoint is object-based, in which case the
    # var_name_to_prev_var_name dictionary should map variable names to
    # checkpoint keys. If the user has specified var_name_to_prev_var_name, we
    # do not override it.
    ms_name_to_prev_ms_name = _get_object_checkpoint_renames(
      ckpt_to_initialize_from, grouped_mht_saveables.keys())

  prev_ms_name_used = set()
  mht_saveables = {}
  ms_warmstarted_count = 0
  for ms_name, ms in six.iteritems(grouped_mht_saveables):
    prev_ms_name = ms_name_to_prev_ms_name.get(ms_name)
    if prev_ms_name:
      prev_ms_name_used.add(ms_name)
    ms_warmstarted_count += 1
    logging.debug("Warm-starting MutableHashTable: {}; prev_ms_name: {}"
                  .format(ms_name, prev_ms_name or "Unchanged"))
    if len(ms) > 1:
      raise ValueError("At least two MutableHashTables have the same name: {}"
                       .format(ms_name))
    ms = ms[0]
    current_ms_name = _infer_mht_saveable_name(ms)
    _ms_name = prev_ms_name if prev_ms_name else current_ms_name
    mht_saveables[_ms_name] = ms

  checkpoint_utils.init_mht_saveable_from_checkpoint(ckpt_to_initialize_from,
                                                     mht_saveables)

  logging.info("Warm-started %d MutableHashTables.", ms_warmstarted_count)

  return prev_ms_name_used, set(grouped_mht_saveables.keys())
