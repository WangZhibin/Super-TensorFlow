...
@tf_export(v1=[
    "initializers.dynamic_embedding_variables_initializer",
    "dynamic_embedding_variables_initializer"
])
def dynamic_embedding_variables_initializer(
    name="init_all_dynamic_embedding_variables"):
  """Returns an Op that initializes all dynamic_embedding_ops.Varaibles of the
  default graph.
  ...
  Returns:
    An Op that initializes all dynamic_embedding_ops.Variables.  Note that if
    there are not dynamic_embedding_ops.Variables the returned Op is a NoOp.
  """
  if context.executing_eagerly():
    return control_flow_ops.no_op(
        name="dynamic_embedding_variables_initializer")
  else:
    initializers = ops.get_collection(
        ops.GraphKeys.DYNAMIC_EMBEDDING_VARIABLE_INITIALIZERS)
    if initializers:
...
class Variable(trackable.TrackableResource):
  ...
  def __init__(self,
    ...
    ops.add_to_collection(ops.GraphKeys.DYNAMIC_EMBEDDING_VARIABLES, self)
  ...
  with ops.name_scope(full_name):
    ...
    with ops.colocate_with(None, ignore_existing=True):
      ...
      trainable_ = resource_variable_ops.TrainableWrapper(
          ...
          update_mode=UpdateMode.CURRENT_SETTING,
...
def safe_embedding_lookup_sparse(
  ...
  with ops.name_scope(full_name + "/"):
    ...
    if type(original_rank) != int or original_rank > 2:
    ...
    sparse_ids, is_row_empty = sparse_ops.sparse_fill_empty_rows(
        sparse_ids, default_id or
        ("" if sparse_ids.dtype == dtypes.string else 0))
    if sparse_weights is not None:
...
class UpdateMode(object):
  """ The global config of training mode.

  'CONVERGENCE_PRIORITY': means that the latest sparse weights will be looked up
    again from parameter servers before applying gradients, that will hurt
    slightly the performance(about 5%~7% by some testing).
    The train loop will be:
    `ids->lookup->[weights]->forward->computing_gradients->lookup
                                                              |
                                                              v
                                apply_gradients<-[latest weights]`

  'SPEED_PRIORITY': means that no extra action will be taken before applying
    gradients, the sparse weights are only be looked up one time at the
    beginning of the training loop, that will hurt the convergence at small
    dataset:
    The train loop will be:
    `ids->lookup->[weights]->forward->computing_gradients->apply_gradients`
                     |                                      ^
                     |______________________________________|

  NOTICE: In long-term training, the convergence of two modes have no
  ...
  # The default setting is convergence priority mode.
  CURRENT_SETTING = CONVERGENCE_PRIORITY


@tf_export("dynamic_embedding.enable_convergence_priority")
def enable_convergence_priority():
  """ Enable convergence mode.
  ...
  'CONVERGENCE_PRIORITY': means that the latest sparse weights will be looked up
    ...
    `ids->lookup->[weights]->forward->computing_gradients->lookup
                                ...
                                apply_gradients<-[latest weights]`
  """
  UpdateMode.CURRENT_SETTING = UpdateMode.CONVERGENCE_PRIORITY


@tf_export("dynamic_embedding.enable_speed_priority")
def enable_speed_priority():
  """ Enable speed priority mode.
  ...
  'SPEED_PRIORITY': means that no extra action will be taken before applying
    ...
    `ids->lookup->[weights]->forward->computing_gradients->apply_gradients`
                     ...
                     |______________________________________|
  """
  UpdateMode.CURRENT_SETTING = UpdateMode.SPEED_PRIORITY


@tf_export("dynamic_embedding.get_update_mode")
def get_update_mode():
  """ get update mode.

  Returns:
    A string: `convergence_priority` if CONVERGENCE_PRIORITY or 'speed_priority'
  """
  return UpdateMode.CURRENT_SETTING
