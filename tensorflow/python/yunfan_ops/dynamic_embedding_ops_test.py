...
class DynamicEmbeddingOpTest(test.TestCase):
  ...
  def test_variable(self):
    ...
    for key_dtype, value_dtype, dim in itertools.product(
        [dtypes.int64], [dtypes.float32, dtypes.half, dtypes.int8], dim_list):
      id += 1
...
class EmbeddingLookupTest(test.TestCase):
  ...
  def test_sharded_multi_lookup_strings_on_one_variable(self):
    if test_util.is_gpu_available():
      self.skipTest("string keys is supported on GPU.")
    embeddings = deo.get_variable("t340-string",
                                  dtypes.string,
                                  dtypes.float32,
                                  devices=_get_devices() * 3,
                                  initializer=2.0)
    ...
    ids = constant_op.constant(
        ["famale", "covid-19", "apple", "politic", "banana"],
        dtype=dtypes.string)
    vals = constant_op.constant([[0.], [1.], [2.], [3.], [4.]],
                                dtype=dtypes.float32)
    new_vals = constant_op.constant([[10.], [11.], [12.], [13.], [14.]],
    ...
    ids0 = constant_op.constant(["covid-19", "politic", "apple"],
                                dtype=dtypes.string)
    ids1 = constant_op.constant(["politic", "banana"], dtype=dtypes.string)

    embedding0 = deo.embedding_lookup(embeddings, ids0)
    embedding1 = deo.embedding_lookup(embeddings, ids1)

    table0_size = embeddings.tables[0].size()
    table1_size = embeddings.tables[1].size()
    table2_size = embeddings.tables[2].size()
    ...
    with self.session(use_gpu=False, config=default_config):
      ...
      self.assertAllClose(table0_size.eval(), 2)
      self.assertAllClose(table1_size.eval(), 2)
      self.assertAllClose(table2_size.eval(), 1)
