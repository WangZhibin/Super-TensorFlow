...
def _next_run_step_config(keys_type_list=[dtypes.int64]):
  ...
  for num_shards, k_dtype, d_dtype, initial_mode, dim, run_step in \
      itertools.product(
         ...
         [10]):
    run_id += 1
    yield run_id, num_shards, k_dtype, d_dtype, initial_mode, dim, run_step
...
class CommonTrainableTestV1Base(object):
  ...
  @test_util.deprecated_graph_mode_only
  def test_lamb_minimize_trainable(self):
    base_opt = lamb.LAMBOptimizer(0.1)
    test_opt = lamb.LAMBOptimizer(0.1)
    self.common_minimize_trainable(base_opt, test_opt, name='lamb')
...
class EmbeddingLookupSparseTrainableV2Test(test.TestCase,
  ...
  def common_minimize_trainable_v2(self, base_opt, test_opt, name):
    ...
    for run_id, num_shards, k_dtype, d_dtype, initial_mode, dim, run_step \
      ...
      def test_fn():
        embeddings = deo.get_variable('t1030-v2-' + name + str(run_id),
                                      key_dtype=k_dtype,
                                      value_dtype=d_dtype,
                                      devices=_get_devices() * num_shards,
                                      initializer=1.,
...
@test_util.deprecated_graph_mode_only
class UpdateModeTest(test.TestCase):
  """Tests UpdateMode."""
